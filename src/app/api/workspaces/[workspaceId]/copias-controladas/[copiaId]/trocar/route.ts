...
import { getCopiaControladaOrThrow, trocarCopiaControlada } from "@/services/copiaControladaService";
...
export const POST = withErrorHandling(async (_req: Request, { params }: Params) => {
  ...
  const nova = await trocarCopiaControlada(workspaceId, user.id, copiaId);
  return NextResponse.json({ data: nova }, { status: 201 });
});
