...
export const workspaceMemberEmailUpdateSchema = z.object({
  email: z.string().trim().email(),
});

export const workspaceMemberNameUpdateSchema = z.object({
  nome: z.string().trim().min(1, "O nome não pode ficar em branco.").max(200),
});
